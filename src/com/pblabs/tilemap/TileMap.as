...
package com.pblabs.tilemap {
	...
	public class TileMap extends SimpleSpatialComponent {
		...
		public override function get worldExtents():Rectangle { return new Rectangle(0, 0, width * tileSize.x, height * tileSize.y); }
		...
		public override function castRay(start:Point, end:Point, flags:ObjectType, info:RayHitInfo):Boolean {
			...
			while (curPos.x >= 0 && curPos.y >= 0 && curPos.x <= (width + 1) && curPos.y <= (height + 1)) {
				if (tMax.x < tMax.y) {
					...
					if (flagMap.getTileFlags(nextPos.x, nextPos.y) & checkMask && rayVsBox(start, end, getTileBounds(nextPos), info))
						return true;
